...
import com.melonstudios.bmnw.entity.custom.LittleBoyEntity;
...
@OnlyIn(Dist.CLIENT)
public class LittleBoyRenderer extends EntityRenderer<LittleBoyEntity> {
    ...
    public LittleBoyRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(LittleBoyEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {

        float f = (partialTick) / 200f;
        poseStack.pushPose();
        poseStack.translate(0, 0, 0);
        renderRays(poseStack, f, buffer.getBuffer(RenderType.dragonRays()), 4);
        renderRays(poseStack, f, buffer.getBuffer(RenderType.dragonRaysDepth()), 4);
        poseStack.popPose();
    }
    ...
    protected static void renderRays(PoseStack poseStack, float dragonDeathCompletion, VertexConsumer buffer, float sizeMultiplier) {
        ...
        RandomSource randomsource = RandomSource.create("little_boy".hashCode());
    ...
    @Override
    public ResourceLocation getTextureLocation(LittleBoyEntity entity) {
        return DRAGON_LOCATION;
    }

    @Override
    public boolean shouldRender(LittleBoyEntity livingEntity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }
}
