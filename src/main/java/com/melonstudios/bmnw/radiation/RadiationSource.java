package com.melonstudios.bmnw.radiation;

import com.melonstudios.bmnw.hazard.HazardRegistry;
import com.melonstudios.bmnw.misc.DoubleHolderThing;
...
public class RadiationSource {
    ...
    public float getImpact(LivingEntity entity) {
        ...
        DoubleHolderThing dht = new DoubleHolderThing(Math.max(armorProtect, 0));
        Vec3 end = new Vec3(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
        Vec3 start = entity.getPosition(1).add(0, entity.getBbHeight() / 2, 0);

        Level level = entity.level();

        manager.iterateBlocks(start, end, (pos) -> {
            dht.multiply(ShieldingValues.getShieldingModifier(level.getBlockState(pos)));
            if (level.getFluidState(pos).is(FluidTags.WATER)) dht.multiply(0.25);
        });

        return (float) (radiation * dht.value);
    }
}