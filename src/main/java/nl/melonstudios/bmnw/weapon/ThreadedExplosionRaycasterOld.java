...
import nl.melonstudios.bmnw.weapon.math.RaycastOld;
...
public class ThreadedExplosionRaycasterOld {
    ...
    public ThreadedExplosionRaycasterOld(ServerLevel level, Position source, float initialStrength) {
        this(level, new Vector3f((float) source.x(), (float) source.y(), (float) source.z()), initialStrength);
    }
    public ThreadedExplosionRaycasterOld(ServerLevel level, Vector3fc source, float initialStrength) {
        this.level = level;
    ...
    public void startRaycasting() {
        ...
        try {
            ...
            RaycastOld raycast = new RaycastOld(this.source);
