...
public class AlloyingRecipes {
    ...
    public void addRecipe(IngredientPair pair, ItemStack result) {
    ...
    public List<AlloyingRecipe> getJEIRecipeList() {
        ...
        for (Map.Entry<IngredientPair, ItemStack> entry : this.recipes.entrySet()) {
            list.add(new AlloyingRecipe(entry.getKey().ingredient1(), entry.getKey().ingredient2(), entry.getValue()));
        }
        return list;
    }
}
